...
func Test_apiKeyNeedsRecreation(t *testing.T) {
	tests := []struct {
		name         string
		...
		{
			name:         "nil metadata requires recreation",
			apiKey:       &esclient.APIKey{Metadata: nil},
			expectedHash: "hash123",
			want:         true,
		},
		{
			name: "missing managed-by metadata requires recreation",
			apiKey: &esclient.APIKey{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := apiKeyNeedsRecreation(tt.apiKey, tt.expectedHash); got != tt.want {
				t.Errorf("apiKeyNeedsRecreation() = %v, want %v", got, tt.want)
			}
		})
	}
}
