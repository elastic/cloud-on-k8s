...
func MaybeReconcileEmptyFileSettingsSecret(ctx context.Context, c k8s.Client, licenseChecker commonlicense.Checker, es *esv1.Elasticsearch, operatorNamespace string) (bool, error) {
	...
	if fs.Exists() {
		if !fs.IsSettingsCorrupted() {
			...
			//
			// WithAdditiveMetadata: the StackConfigPolicy controller owns additional managed
			// annotations on this Secret (e.g. secure-settings secret references). This Save
			// path does not load those into fs.secureSettingsSources; without additive merging,
			// applyExpectedSecret would strip them and break keystore reconciliation for SCP
			// secure settings (see filesettings.WithAdditiveMetadata / ReconcileClusterSecrets).
