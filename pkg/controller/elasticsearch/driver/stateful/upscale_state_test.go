...
func Test_upscaleState_limitNodesCreation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			state:       &upscaleState{createsAllowed: new(int32(2))},
			...
			wantState:   &upscaleState{createsAllowed: new(int32(2)), recordedCreates: 2},
		},
		{
		...
		},
		{
			...
			wantState:   &upscaleState{createsAllowed: new(int32(1)), recordedCreates: 1},
		},
		{
			name:        "upscale nodes from 3 to 4, but no creates allowed: should limit to 0",
			state:       &upscaleState{createsAllowed: new(int32(0))},
			actual:      sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			ssetToApply: sset.TestSset{Name: "sset", Replicas: 4, Master: true}.Build(),
			wantSset:    sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			wantState:   &upscaleState{createsAllowed: new(int32(0)), recordedCreates: 0},
		},
	}
...
func Test_newUpscaleState(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: &upscaleState{createsAllowed: new(int32(6))}, // 3 maxSurge + 3 expected - 0 actual = 6 creates allowed
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_newUpscaleStateWithChangeBudget(t *testing.T) {
	...
	getTest := func(args args) test {
		...
		return test{
			...
			ctx: upscaleCtx{
				...
				es:        bootstrappedESWithChangeBudget(args.maxSurge, new(int32(0))),
			},
			actual:   actualSsets,
	...
	tests := []test{
		getTest(args{actual: []int{3}, expected: []int{3}, maxSurge: new(int32(0)), createsAllowed: new(int32(0)), name: "3 nodes present, 3 nodes target - no creates allowed"}),
		getTest(args{actual: []int{3, 3}, expected: []int{3, 3}, maxSurge: new(int32(0)), createsAllowed: new(int32(0)), name: "2 ssets, 6 nodes present, 6 nodes target - no creates allowed"}),
		getTest(args{actual: []int{2}, expected: []int{3}, maxSurge: new(int32(0)), createsAllowed: new(int32(1)), name: "2 nodes present, 3 nodes target - 1 create allowed"}),
		getTest(args{actual: []int{}, expected: []int{5}, maxSurge: new(int32(3)), createsAllowed: new(int32(8)), name: "0 nodes present, 5 nodes target, 3 maxSurge - 8 creates allowed"}),
		getTest(args{actual: []int{5}, expected: []int{3}, maxSurge: new(int32(3)), createsAllowed: new(int32(1)), name: "5 nodes present, 3 nodes target, 3 maxSurge - 1 create allowed"}),
		defaultTest,
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_calculateCreatesAllowed(t *testing.T) {
	...
	tests := []args{
		...
		{name: "0 budget, 5->5, want 0", maxSurge: new(int32(0)), actual: 5, expected: 5, want: new(int32(0))},
		{name: "1 budget, 5->5, want 1", maxSurge: new(int32(1)), actual: 5, expected: 5, want: new(int32(1))},
		{name: "2 budget, 5->5, want 2", maxSurge: new(int32(2)), actual: 5, expected: 5, want: new(int32(2))},
		{name: "2 budget, 3->5, want 4", maxSurge: new(int32(2)), actual: 3, expected: 5, want: new(int32(4))},
		{name: "6 budget, 10->5, want 4", maxSurge: new(int32(6)), actual: 10, expected: 5, want: new(int32(1))},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
