...
func Test_upscaleState_limitNodesCreation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			state:       &upscaleState{allowMasterCreation: true, isBootstrapped: true, createsAllowed: pointer.Int32Ptr(2)},
			...
			wantState:   &upscaleState{allowMasterCreation: true, isBootstrapped: true, createsAllowed: pointer.Int32Ptr(2), recordedCreates: 2},
		},
		{
		...
		},
		{
			...
			state:       &upscaleState{allowMasterCreation: true, isBootstrapped: true, createsAllowed: pointer.Int32Ptr(1)},
			actual:      sset.TestSset{Name: "sset", Replicas: 1, Master: true}.Build(),
			ssetToApply: sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			wantSset:    sset.TestSset{Name: "sset", Replicas: 2, Master: true}.Build(),
			wantState:   &upscaleState{allowMasterCreation: false, isBootstrapped: true, createsAllowed: pointer.Int32Ptr(1), recordedCreates: 1},
		},
		{
			name:        "upscale master nodes from 1 to 3 when cluster not yet bootstrapped: should go through",
			state:       &upscaleState{allowMasterCreation: true, isBootstrapped: false, createsAllowed: pointer.Int32Ptr(2)},
			actual:      sset.TestSset{Name: "sset", Replicas: 1, Master: true}.Build(),
			ssetToApply: sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			wantSset:    sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			wantState:   &upscaleState{allowMasterCreation: true, isBootstrapped: false, createsAllowed: pointer.Int32Ptr(2), recordedCreates: 2},
		},
		{
			name:        "upscale masters from 3 to 4, but no creates allowed: should limit to 0",
			state:       &upscaleState{allowMasterCreation: true, isBootstrapped: true, createsAllowed: pointer.Int32Ptr(0)},
			actual:      sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			ssetToApply: sset.TestSset{Name: "sset", Replicas: 4, Master: true}.Build(),
			wantSset:    sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			wantState:   &upscaleState{allowMasterCreation: true, isBootstrapped: true, createsAllowed: pointer.Int32Ptr(0), recordedCreates: 0},
		},
		{
		...
		},
		{
			...
			state:       &upscaleState{allowMasterCreation: true, isBootstrapped: false, createsAllowed: pointer.Int32Ptr(3)},
			actual:      appsv1.StatefulSet{},
			ssetToApply: sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			wantSset:    sset.TestSset{Name: "sset", Replicas: 3, Master: true}.Build(),
			wantState:   &upscaleState{allowMasterCreation: true, isBootstrapped: false, createsAllowed: pointer.Int32Ptr(3), recordedCreates: 3},
		},
		{
...
func Test_newUpscaleStateWithChangeBudget(t *testing.T) {
	...
	getTest := func(args args) test {
		...
		return test{
			...
			ctx: upscaleCtx{
				...
				es:        *bootstrappedESWithChangeBudget(args.maxSurge, pointer.Int32Ptr(0)),
			},
			actual:   actualSsets,
	...
	tests := []test{
		getTest(args{actual: []int{3}, expected: []int{3}, maxSurge: pointer.Int32Ptr(0), createsAllowed: pointer.Int32Ptr(0), name: "3 nodes present, 3 nodes target - no creates allowed"}),
		getTest(args{actual: []int{3, 3}, expected: []int{3, 3}, maxSurge: pointer.Int32Ptr(0), createsAllowed: pointer.Int32Ptr(0), name: "2 ssets, 6 nodes present, 6 nodes target - no creates allowed"}),
		getTest(args{actual: []int{2}, expected: []int{3}, maxSurge: pointer.Int32Ptr(0), createsAllowed: pointer.Int32Ptr(1), name: "2 nodes present, 3 nodes target - 1 create allowed"}),
		getTest(args{actual: []int{}, expected: []int{5}, maxSurge: pointer.Int32Ptr(3), createsAllowed: pointer.Int32Ptr(8), name: "0 nodes present, 5 nodes target, 3 maxSurge - 8 creates allowed"}),
		getTest(args{actual: []int{5}, expected: []int{3}, maxSurge: pointer.Int32Ptr(3), createsAllowed: pointer.Int32Ptr(1), name: "5 nodes present, 3 nodes target, 3 maxSurge - 1 create allowed"}),
		defaultTest,
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_calculateCreatesAllowed(t *testing.T) {
	...
	tests := []args{
		...
		{name: "0 budget, 5->5, want 0", maxSurge: pointer.Int32Ptr(0), actual: 5, expected: 5, want: pointer.Int32Ptr(0)},
		{name: "1 budget, 5->5, want 1", maxSurge: pointer.Int32Ptr(1), actual: 5, expected: 5, want: pointer.Int32Ptr(1)},
		{name: "2 budget, 5->5, want 2", maxSurge: pointer.Int32Ptr(2), actual: 5, expected: 5, want: pointer.Int32Ptr(2)},
		{name: "2 budget, 3->5, want 4", maxSurge: pointer.Int32Ptr(2), actual: 3, expected: 5, want: pointer.Int32Ptr(4)},
		{name: "6 budget, 10->5, want 4", maxSurge: pointer.Int32Ptr(6), actual: 10, expected: 5, want: pointer.Int32Ptr(1)},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
