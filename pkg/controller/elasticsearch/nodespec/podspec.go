...
func BuildPodTemplateSpec(
	...
	builder = builder.
		...
		WithResourcesAndOverrides(DefaultResources, nodeSet.Resources.ContainerResources()).
	...
	builder, err = stackmon.WithMonitoring(ctx, client, builder, es, meta, clientAuthenticationRequired, es.Spec.Monitoring.ElasticAgent)
	if err != nil {
		return corev1.PodTemplateSpec{}, err
	}
