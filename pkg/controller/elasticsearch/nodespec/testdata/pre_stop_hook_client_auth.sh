...
if [ -f "/usr/share/elasticsearch/config/client-cert/tls.crt" ]; then
  # client certificates are mounted by the operator for client authentication
  CLIENT_CERT=("--cert" "/usr/share/elasticsearch/config/client-cert/tls.crt" "--key" "/usr/share/elasticsearch/config/client-cert/tls.key")
elif [ -f "/usr/share/elasticsearch/config/http-certs/tls.crt" ]; then
  ...
  CLIENT_CERT=("--cert" "/usr/share/elasticsearch/config/http-certs/tls.crt" "--key" "/usr/share/elasticsearch/config/http-certs/tls.key")
else
  CLIENT_CERT=()
fi
