...
func Test_validPVCModification(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// A claim that declares only a storage class and no size at all. This is what the
			// autoscaling contract asks users to write, since the size arrives via
			// spec.nodeSets[].resources.storage, and it used to panic in claimsWithoutStorageReq.
