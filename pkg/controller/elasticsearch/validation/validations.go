...
func hasCorrectNodeRoles(es esv1.Elasticsearch) field.ErrorList {
	...
	for i, ns := range es.Spec.NodeSets {
		...
		seenMaster = seenMaster || (cfg.Node.IsConfiguredWithRole(esv1.MasterRole) && !cfg.Node.IsConfiguredWithRole(esv1.VotingOnlyRole) && ns.Count > 0) || es.IsAutoscalingAnnotationSet()
	}
