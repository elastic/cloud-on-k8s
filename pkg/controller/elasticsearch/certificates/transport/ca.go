...
// The second return value carries the previous CA certificate bytes (PEM) when a rotation
// happened within CATransitionGracePeriod ago; callers should include them in the transport
// trust bundle so that pods which have not yet received the new CA can still verify peers.
func ReconcileOrRetrieveCA(
	...
	if customCASecret == nil {
		...
		previousCABytes, err := certificates.GetPreviousCABytes(
			ctx, driver.K8sClient(), esv1.ESNamer, &es, certificates.TransportCAType,
		)
		if err != nil {
			return nil, nil, err
		}

		return ca, previousCABytes, nil
	}
	...
	if err != nil {
		...
		k8s.EmitEvent(driver.Recorder(), &es, corev1.EventTypeWarning, events.EventReasonValidation, events.EventActionParseSecret, err.Error())
		return nil, nil, err
	}
	...
	if err := certificates.ValidateCustomCA(ctx, ca); err != nil {
		...
		k8s.EmitEvent(driver.Recorder(), &es, corev1.EventTypeWarning, events.EventReasonValidation, events.EventActionValidation, validationErr.Error())
