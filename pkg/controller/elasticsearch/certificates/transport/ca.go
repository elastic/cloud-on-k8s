...
func ReconcileOrRetrieveCA(
	...
	if err != nil {
		...
		k8s.EmitEvent(driver.Recorder(), &es, corev1.EventTypeWarning, events.EventReasonValidation, events.EventActionParseSecret, "%s", err.Error())
		return nil, err
	}
	...
	if err := certificates.ValidateCustomCA(ctx, ca); err != nil {
		...
		k8s.EmitEvent(driver.Recorder(), &es, corev1.EventTypeWarning, events.EventReasonValidation, events.EventActionValidation, "%s", validationErr.Error())
