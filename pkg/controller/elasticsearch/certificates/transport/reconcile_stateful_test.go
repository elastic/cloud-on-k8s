...
func TestReconcileTransportCertificatesSecrets(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := ReconcileStatefulTransportCertificatesSecrets(context.Background(), k8sClient, tt.args.ca, tt.args.extraCA, *tt.args.es, tt.args.rotationParams, md)
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				es:       testES,
				ssetName: esv1.PodsControllerResourceName(testEsName, "sset1"),
			},
...
func Test_ensureTransportCertificateSecretExists(t *testing.T) {
	defaultSecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      esv1.StatefulSetTransportCertificatesSecret(esv1.PodsControllerResourceName(testES.Name, "sset1")),
			Namespace: testES.Namespace,
			Labels: map[string]string{
				label.ClusterNameLabelName:     testES.Name,
				"common.k8s.elastic.co/type":   "elasticsearch",
				label.StatefulSetNameLabelName: esv1.PodsControllerResourceName(testES.Name, "sset1"),
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := ensureTransportCertificatesSecretExists(context.Background(), tt.args.c, tt.args.owner, esv1.PodsControllerResourceName(testES.Name, "sset1"), md)
			if (err != nil) != tt.wantErr {
