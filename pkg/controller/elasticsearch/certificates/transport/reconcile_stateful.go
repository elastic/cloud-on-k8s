...
// ReconcileStatefulTransportCertificatesSecrets reconciles the secret containing transport certificates for all nodes in the
// stateful Elasticsearch cluster.
// Secrets which are not used anymore are deleted as part of the downscale process.
func ReconcileStatefulTransportCertificatesSecrets(
	ctx context.Context,
	c k8s.Client,
	ca *certificates.CA,
	additionalCAs []byte,
	...
	for _, nodeSet := range es.Spec.NodeSets {
		ssets.Add(esv1.PodsControllerResourceName(es.Name, nodeSet.Name))
	}
