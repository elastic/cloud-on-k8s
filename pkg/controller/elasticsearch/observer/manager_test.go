...
func TestManager_Observe(t *testing.T) {
	fakeClient := fakeEsClient200(t, client.UserAuth{})
	fakeClientWithDifferentUser := fakeEsClient200(t, client.UserAuth{Name: "name", Password: "another-one"})
	tests := []struct {
		name                   string
...
func TestManager_StopObserving(t *testing.T) {
	esClient := fakeEsClient200(t, client.UserAuth{})
	tests := []struct {
		name                       string
...
func TestManager_AddObservationListener(t *testing.T) {
	...
	// observe 2 clusters
	obs1 := m.Observe(cluster("cluster1"), fakeEsClient200(t, client.UserAuth{}))
	defer obs1.Stop()
	obs2 := m.Observe(cluster("cluster2"), fakeEsClient200(t, client.UserAuth{}))
	defer obs2.Stop()
