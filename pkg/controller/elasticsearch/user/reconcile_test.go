...
func TestReconcileUsersAndRoles(t *testing.T) {
	...
	controllerUser, err := ReconcileUsersAndRoles(context.Background(), c, sampleEsWithAuth, initDynamicWatches(), record.NewFakeRecorder(10), testPasswordHasher, fixtures.TestRandomGenerator(), metadata.Metadata{})
...
func Test_aggregateFileRealm(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fileRealm, controllerUser, err := aggregateFileRealm(context.Background(), c, tt.es, initDynamicWatches(), record.NewFakeRecorder(10), testPasswordHasher, fixtures.TestRandomGenerator(), metadata.Metadata{})
			require.NoError(t, err)
			require.NotEmpty(t, controllerUser.Password)
