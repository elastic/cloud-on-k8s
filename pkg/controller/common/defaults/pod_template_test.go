...
func TestPodTemplateBuilder_WithPreferredNodeAffinityMatchExpressions(t *testing.T) {
	tests := []struct {
		name         string
		podTemplate  corev1.PodTemplateSpec
		requirements []corev1.NodeSelectorRequirement
		...
		{
			name:        "creates preferred node affinity when absent",
			podTemplate: corev1.PodTemplateSpec{},
			requirements: []corev1.NodeSelectorRequirement{
				{Key: "topology.kubernetes.io/zone", Operator: corev1.NodeSelectorOpExists},
			},
		...
		{
			name: "appends preferred term when key missing",
			podTemplate: corev1.PodTemplateSpec{
				Spec: corev1.PodSpec{
					Affinity: &corev1.Affinity{
						NodeAffinity: &corev1.NodeAffinity{
		...
		{
			name: "does not duplicate preferred term when key already exists",
			podTemplate: corev1.PodTemplateSpec{
				Spec: corev1.PodSpec{
					Affinity: &corev1.Affinity{
						NodeAffinity: &corev1.NodeAffinity{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			builder := NewPodTemplateBuilder(tt.podTemplate, "elasticsearch")
			got := builder.WithPreferredNodeAffinityMatchExpressions(tt.requirements...).PodTemplate.Spec.Affinity
			if !reflect.DeepEqual(tt.wantAffinity, got) {
				t.Errorf("PodTemplateBuilder.WithPreferredNodeAffinityMatchExpressions() = %v, want %v", got, tt.wantAffinity)
			}
		})
	}
}
