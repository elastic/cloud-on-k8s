...
func TestImageRepository(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:    "AutoOps image without suffix",
			image:   AutoOpsAgentImage,
			version: "9.4.2",
			want:    testRegistry + "/elastic-agent/elastic-otel-collector:9.4.2",
		},
		{
			name:    "AutoOps image with -wolfi suffix selects wolfi variant",
			image:   AutoOpsAgentImage,
			version: "9.4.2",
			suffix:  "-wolfi",
			want:    testRegistry + "/elastic-agent/elastic-otel-collector-wolfi:9.4.2",
		},
		{
			name:    "Enterprise Search image without suffix",
			image:   EnterpriseSearchImage,
			version: "8.16.0",
			want:    testRegistry + "/enterprise-search/enterprise-search:8.16.0",
		},
		{
			name:    "Enterprise Search image ignores -wolfi suffix (already wolfi-native)",
			image:   EnterpriseSearchImage,
			version: "8.16.0",
			suffix:  "-wolfi",
			want:    testRegistry + "/enterprise-search/enterprise-search:8.16.0",
		...
		{
			name:    "Enterprise Search image with non-wolfi custom suffix is applied",
			image:   EnterpriseSearchImage,
			version: "8.16.0",
			suffix:  "-custom",
			want:    testRegistry + "/enterprise-search/enterprise-search-custom:8.16.0",
		},
		{
		...
		{
			name:    "Package registry image ignores -wolfi suffix (already wolfi-native)",
			image:   PackageRegistryImage,
			version: "9.0.0",
			suffix:  "-wolfi",
			want:    testRegistry + "/package-registry/distribution:lite-9.0.0",
		},
		{
