...
func Test_webhook_Create(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantAllowed: false,
			wantMessage: "Unsupported version: version 8.11.1 is lower than the lowest supported version of 8.12.0",
		},
		{
		...
		},
		{
			...
			wantAllowed: false,
			wantMessage: "Invalid association reference: specify name or secretName, not both",
		},
		{
			...
			wantAllowed: false,
			wantMessage: "serviceName or namespace can only be used in combination with name, not with secretName",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			v := commonwebhook.NewResourceValidator[*v1alpha1.Logstash](nil, []string{"ns"}, inner)
			wh := admission.WithValidator[*v1alpha1.Logstash](k8s.Scheme(), v)
...
func Test_webhook_Update(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantAllowed: false,
			wantMessage: "Version downgrades are not supported",
		},
		{
