...
func (r *ReconcileApmServer) reconcileApmServerDeployment(
	...
	if common.IsOrchestrationPaused(as) {
		return state, deployment.SetPausedConditionAndEmitEvent(ctx, r.K8sClient(), r.recorder, as, deploy, &as.Status.Conditions)
	}

	deployment.MaybeResetPausedCondition(&as.Status.Conditions)
