...
func (d *driver) Reconcile(
	...
	if common.IsOrchestrationPaused(kb) {
		return results.WithError(deployment.SetPausedConditionAndEmitEvent(ctx, d.client, d.recorder, kb, expectedDp, &kb.Status.Conditions))
	}

	deployment.MaybeResetPausedCondition(&kb.Status.Conditions)
