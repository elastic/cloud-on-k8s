...
func filterWithUserProvidedClientCert(associations []commonv1.Association) []commonv1.Association {
	...
	for _, a := range associations {
		if a.AssociationRef().ClientCertificateSecretName != "" {
			r = append(r, a)
		}
	}
	return r
...
func (r *Reconciler) reconcileClientCertificate(
	...
	extraLabels := map[string]string{
		reconciler.SoftOwnerNameLabel:      ref.Name,
		reconciler.SoftOwnerNamespaceLabel: ref.Namespace,
		reconciler.SoftOwnerKindLabel:      r.ReferencedResourceKind,
		labels.ClientCertificateLabelName:  "true",
	}

	if userSecretName := association.AssociationRef().ClientCertificateSecretName; userSecretName != "" {
		if err := r.reconcileUserProvidedClientCert(ctx, association, assocMeta, secretName, userSecretName, extraLabels); err != nil {
			return "", results.WithError(err)
		}
		return secretName, results
	}

	return r.reconcileManagedClientCert(ctx, association, assocMeta, secretName, extraLabels)
}

// reconcileManagedClientCert creates or updates a self-signed client certificate.
func (r *Reconciler) reconcileManagedClientCert(
	ctx context.Context,
	association commonv1.Association,
	assocMeta metadata.Metadata,
	secretName string,
...
func (r *Reconciler) reconcileUserProvidedClientCert(
	ctx context.Context,
	association commonv1.Association,
	assocMeta metadata.Metadata,
	...
	userSecret, err := k8s.GetSecretIfExists(ctx, r.Client, types.NamespacedName{
	...
	_, err = reconciler.ReconcileSecret(ctx, r.Client, expected, association.Associated())
	return err
}
