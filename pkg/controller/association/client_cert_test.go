...
func TestFilterWithUserProvidedClientCert(t *testing.T) {
	...
	t.Run("returns only associations with user-provided cert", func(t *testing.T) {
		...
		require.Equal(t, "user-provided-cert", filtered[0].AssociationRef().ClientCertificateSecretName)
	})
...
func TestClientCertSecretName(t *testing.T) {
	...
	t.Run("different refs produce different names", func(t *testing.T) {
		...
		kibana2.Spec.ElasticsearchRef = commonv1.ObjectSelector{
			Name:      "other-es",
			Namespace: "es-ns",
		}
...
func TestReconciler_Reconcile_ClientCertOrphanCleanup(t *testing.T) {
	...
	kbWithEsA.Spec = kbv1.KibanaSpec{
		Version:          "7.7.0",
		ElasticsearchRef: commonv1.ObjectSelector{Name: esA.Name, Namespace: esA.Namespace},
	}
	...
	updatedKb.Spec.ElasticsearchRef = commonv1.ObjectSelector{Name: esB.Name, Namespace: esB.Namespace}
