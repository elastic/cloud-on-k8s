...
func isFleetServerClientAuthRequired(params Params) (bool, string, error) {
	...
	if !agentv1alpha1.FleetServerClientAuthSupported(params.AgentVersion) {
		if specEnabled {
			return false, "spec.http.tls.client.authentication is ineffective: Fleet Server client certificate authentication requires Elastic Agent 9.3.6+, 9.4.3+, or 9.5.0+", nil
		}
		return false, "", nil
	}
