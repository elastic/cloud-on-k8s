...
import (
	...
	"github.com/elastic/cloud-on-k8s/v3/pkg/controller/beat/auditbeat"
	...
	"github.com/elastic/cloud-on-k8s/v3/pkg/controller/beat/journalbeat"
	"github.com/elastic/cloud-on-k8s/v3/pkg/controller/beat/metricbeat"
	"github.com/elastic/cloud-on-k8s/v3/pkg/controller/beat/otherbeat"
	"github.com/elastic/cloud-on-k8s/v3/pkg/controller/beat/packetbeat"
...
func (r *ReconcileBeat) validate(ctx context.Context, beat *beatv1beta1.Beat) error {
	...
	if errs := commonnodelabels.ValidateAnnotation(beat.Annotations, r.ExposedNodeLabels); len(errs) > 0 {
		err := apierrors.NewInvalid(schema.GroupKind{Group: beatv1beta1.GroupVersion.Group, Kind: beatv1beta1.Kind}, beat.Name, errs)
		ulog.FromContext(ctx).Error(err, "Validation failed")
		k8s.MaybeEmitErrorEvent(r.recorder, err, beat, events.EventReasonValidation, events.EventActionValidation, err.Error())
		return tracing.CaptureError(vctx, err)
	}
	for _, warning := range warnings {
		k8s.EmitEvent(r.recorder, beat, corev1.EventTypeWarning, events.EventReasonValidation, events.EventActionValidation, warning)
	}
