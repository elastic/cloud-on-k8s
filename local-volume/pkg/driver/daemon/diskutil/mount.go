...
func BindMount(newCmd cmdutil.FactoryFunc, source string, target string) error {
...
func MountDevice(newCmd cmdutil.FactoryFunc, devicePath string, mountPath string) error {
	cmd := newCmd("mount", devicePath, mountPath)
	return cmdutil.RunCmd(cmd)
}

// Unmount unmounts the filesystem at the given mountPath
func Unmount(newCmd cmdutil.FactoryFunc, mountPath string) error {
