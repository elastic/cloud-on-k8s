...
Create certificates to secure external communications. We are creating two certificates so that Elasticsearch can be accessed by the `elasticsearch.hulk` domain name and Kibana can be accessed by the `kibana.hulk` domain name.

[source,yaml]
----
include::01-certificates.yaml[]
----
