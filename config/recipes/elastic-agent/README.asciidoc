...
The provided example is functionally identical to the previous section but runs the Elastic Agent processes (both the Elastic Agent running as the Fleet server and the Elastic Agent connected to Fleet) as the non-root user by utilizing a Daemonset to ensure directory and file permissions. *Note* The Daemonset itself must run as root to ensure permissions.
