...
1. A self-signed `Issuer` (only purpose: sign the CA certificate itself)
...
The HTTP layer is pointed at the cert-manager-issued secret via `spec.http.tls.certificate.secretName`. ECK reads `tls.crt`, `tls.key` and `ca.crt` from it and configures `xpack.security.http.ssl.*` automatically. Because `ca.crt` contains the shared CA, Elasticsearch trusts client certificates signed by that CA.
...
Standalone Elastic Agents and Beats do not use Fleet -- they receive their configuration directly from the CRD's `spec.config` field. ECK generates the output section automatically from the association (`elasticsearchRefs` for agents, `elasticsearchRef` for beats), including hosts, credentials and CA. The user-provided config is then merged on top of this generated config. This means you can add `ssl.certificate` and `ssl.key` to the output in `spec.config` to extend the ECK-generated output with client certificate settings, without having to re-specify hosts, credentials or CA paths.
...
Deploys Elasticsearch, Kibana, Fleet Server and a Fleet-managed Elastic Agent, all with mTLS enabled. A single CA signs all certificates. Each component presents a client certificate during TLS handshakes with Elasticsearch and Fleet Server, in addition to the standard token/password authentication that ECK manages automatically.

==== Component certificates

- **Elasticsearch**: server auth only -- it serves TLS but does not need to present a client cert anywhere.
...
In addition to presenting its own client cert to Elasticsearch (see <<Kibana>> above), Kibana pre-configures the default Fleet output via `xpack.fleet.outputs`. This output is pushed to all enrolled agents and includes `ssl.certificate`, `ssl.key` and `ssl.certificate_authorities` pointing to where the agent's client cert will be mounted (`/usr/share/elastic-agent/config/client-certs/`). This is a preconfigured (immutable) output -- it cannot be changed via the Fleet API, only through the Kibana config. It applies to ALL agents, including Fleet Server's own monitoring agent.
...
Additionally, the Fleet output (configured in Kibana) specifies cert paths at `/usr/share/elastic-agent/config/client-certs/`. Fleet Server's own monitoring agent uses this output too, but Fleet Server's certs live at a different path (`/usr/share/fleet-server/config/http-certs/`). To solve this path mismatch, the same HTTP cert secret is mounted a second time at the agent-expected path via the `fleet-server-client-certs` volume.

==== Elastic Agent

The agent's client certificate (`elastic-agent-client-certs-cm`) is mounted at `/usr/share/elastic-agent/config/client-certs/`. It is used in two places:
...
2. **Agent -> Elasticsearch**: After enrollment, the agent receives its policy from Fleet. The default output (preconfigured in `xpack.fleet.outputs`) includes `ssl.certificate`, `ssl.key` and `ssl.certificate_authorities` pointing to the same `/usr/share/elastic-agent/config/client-certs/` path, ensuring the agent presents the client cert when shipping data to Elasticsearch.
...
kubectl exec $AGENT_POD -- curl -svk \
  --cert /usr/share/elastic-agent/config/client-certs/tls.crt \
  --key /usr/share/elastic-agent/config/client-certs/tls.key \
...
Deploys Elasticsearch, Kibana and a standalone Elastic Agent (without Fleet) with mTLS enabled between all components and Elasticsearch.

Because there is no Fleet Server, the setup is simpler than the Fleet-managed recipe. Only three certificates are needed: the Elasticsearch server cert, the Kibana server+client cert and the agent's client cert. Kibana presents its HTTP certificate as a client cert to Elasticsearch. The agent's client certificate is mounted at `/usr/share/elastic-agent/config/client-certs/` and referenced via `outputs.default.ssl.certificate` and `outputs.default.ssl.key` in `spec.config`, which ECK merges on top of the auto-generated output (see <<Config merge for standalone components (Agents and Beats)>> above).
...
Deploys Elasticsearch, Kibana and a Metricbeat DaemonSet with mTLS enabled between all components and Elasticsearch. Kibana presents its HTTP certificate as a client cert to Elasticsearch. Metricbeat's client certificate is mounted at `/usr/share/metricbeat/config/client-certs/` and referenced via `output.elasticsearch.ssl.certificate` and `output.elasticsearch.ssl.key` in `spec.config`, which ECK merges on top of the auto-generated output (see <<Config merge for standalone components (Agents and Beats)>> above).
