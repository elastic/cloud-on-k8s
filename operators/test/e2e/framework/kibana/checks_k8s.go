...
func CheckKibanaDeployment(b Builder, k *framework.K8sClient) framework.TestStep {
...
func CheckKibanaPodsCount(b Builder, k *framework.K8sClient) framework.TestStep {
	return framework.TestStep{
		Name: "Kibana pods count should match the expected one",
		Test: framework.Eventually(func() error {
			return k.CheckPodCount(framework.KibanaPodListOptions(b.Kibana.Name), int(b.Kibana.Spec.NodeCount))
		}),
	}
}

// CheckKibanaPodsRunning checks that all Kibana pods for the given Kibana are running
func CheckKibanaPodsRunning(b Builder, k *framework.K8sClient) framework.TestStep {
	return framework.TestStep{
		Name: "Kibana pods should eventually be running",
		Test: framework.Eventually(func() error {
			pods, err := k.GetPods(framework.KibanaPodListOptions(b.Kibana.Name))
			if err != nil {
				return err
			}
			for _, p := range pods {
