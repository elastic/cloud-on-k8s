...
func CheckApmServerDeployment(b Builder, k *framework.K8sClient) framework.TestStep {
...
func CheckApmServerPodsCount(b Builder, k *framework.K8sClient) framework.TestStep {
	return framework.TestStep{
		Name: "ApmServer pods count should match the expected one",
		Test: framework.Eventually(func() error {
			return k.CheckPodCount(framework.ApmServerPodListOptions(b.ApmServer.Name), int(b.ApmServer.Spec.NodeCount))
		}),
	}
}

// CheckApmServerPodsRunning checks that all APM Server pods for the given APM Server are running
func CheckApmServerPodsRunning(b Builder, k *framework.K8sClient) framework.TestStep {
	return framework.TestStep{
		Name: "ApmServer pods should eventually be running",
		Test: framework.Eventually(func() error {
			pods, err := k.GetPods(framework.ApmServerPodListOptions(b.ApmServer.Name))
			if err != nil {
				return err
			}
			for _, p := range pods {
