...
func processOngoingRestarts(restartContext RestartContext) (done bool, err error) {
	...
	log.V(1).Info("Pods annotated for restart", "namespace", restartContext.Cluster.Namespace, "es_name", restartContext.Cluster.Name)
...
func schedulePodsRestart(c k8s.Client, pods pod.PodsWithConfig, strategy Strategy) (int, error) {
	...
	for _, p := range pods {
		if isAnnotatedForRestart(p.Pod) {
			log.V(1).Info("Pod already in a restart phase", "namespace", p.Pod.Namespace, "pod_name", p.Pod.Name)
			continue
		}
