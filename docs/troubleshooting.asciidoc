ifdef::env-github[]
****
link:https://www.elastic.co/guide/en/cloud-on-k8s/master/k8s-troubleshooting.html[View this document online]
****
endif::[]
...
If an Elasticsearch node does not come up, it is probably because Kubernetes cannot schedule the associated Pod.

Start by looking at the StatefulSets to check that the current number of replicas match the desired number of replicas.
...
Next, check the Pod status. If a pod fails to reach the `Running` status after a few seconds, something could be preventing it from being scheduled or starting up:
