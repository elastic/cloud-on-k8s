...
If an Elasticsearch node does not come up, it's probably because Kubernetes cannot schedule the associated Pod.

Start by looking at the StatefulSets to check that the current number of replicas matches the desired number of replicas.
...
Next, check the Pod status: if a pod is not `Running` after a few seconds, something is wrong:
