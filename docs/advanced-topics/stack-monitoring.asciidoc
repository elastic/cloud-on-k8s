  ...
  monitoring:
    ...
    logs:
      ...
      - name: monitoring-logs
        serviceName: monitoring-logs-es-coordinating-nodes <3>
...
<2> The use of `namespace` is optional if the Secret describing the monitoring Elasticsearch cluster and the monitored Elasticsearch cluster are running in the same namespace.
<3> The `serviceName` can only be used in conjunction with `name`, not with `secretName`.
...
stringData:
  url: https://mon1.es.eu-west-3.aws.elastic-cloud.com:9243
  username: monitoring-user
  password: @j5SiN;<7yLJQz4<7:g:ap7hBb:2slbtfZ}?[PyO
