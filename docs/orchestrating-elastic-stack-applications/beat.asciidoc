...
Deploys Metricbeat as a DaemonSet that monitors the host resource usage (CPU, memory, network, filesystem), OpenShift resources (Nodes, Pods, Containers, Volumes), API Server and Filebeat using autodiscover. Deploys one monitored Elasticsearch cluster and one monitoring Elasticsearch cluster.Deploys one monitored Elasticsearch cluster and one monitoring Elasticsearch cluster. Deploys an Elasticsearch cluster and Kibana to centralize data collection.
