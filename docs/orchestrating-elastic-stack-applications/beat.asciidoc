...
In the Beat YAML specification you can change settings and version upgrades. ECK detects those changes and ensures a smooth rolling upgrade of all Beat Pods. Depending on the specification settings that you used, ECK will set the <<{p}-beat-set-beat-output,output>> part of the config, perform Kibana dashboard <<{p}-beat-set-up-kibana-dashboards,setup>>, restart Beats on certificates rollover and set up the Beats <<{p}-beat-secrets-keystore-for-secure-settings,keystore>>.
...
Beats offer a secrets keystore for sensitive settings, like passwords. This avoids storing them in the configuration directly.
