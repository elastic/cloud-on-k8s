...
NOTE: When you change the `clusterIP` setting of the service, ECK deletes and re-creates the service, as `clusterIP` is an immutable field. Typically this does not affect connectivity, as the transport module uses long-lived TCP connections, but may cause a small network disruption between Elasticsearch nodes.
