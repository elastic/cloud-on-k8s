...
NOTE: The remote clusters feature requires a valid Enterprise license or Enterprise trial license. See <<{p}-licensing, the license documentation>> for details how to install your license.

You can create a remote cluster connection to another Elasticsearch cluster inside the same Kubernetes cluster by specifying the `remoteClusters` attribute in your Elasticsearch spec.
