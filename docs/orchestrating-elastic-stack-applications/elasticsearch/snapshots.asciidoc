...
=== Basic snapshot repository setup for Azure - example

. Ensure theere is a configured Storage Account in Azure and have created an Access Key for that Storage Account.
+
. Ensure a Container within the Storage Account is also configured, in this example we will call it `azuresnaphots`
+
