...
* `count` corresponds to the number of replicas in the StatefulSet. Each replica leads to the creation of a Pod, which corresponds to a single Elasticsearch node.
...
** Is all the Elasticsearch nodes of a NodeSet are unavailable, probably caused by a misconfiguration, the operator ignores the cluster health and upgrades nodes of the NodeSet.
