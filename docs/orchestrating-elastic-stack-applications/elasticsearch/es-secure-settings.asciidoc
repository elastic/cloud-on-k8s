...
TIP: Note that by default https://kubernetes.io/docs/concepts/configuration/secret/[Kubernetes secrets] are expecting the value to be base64 encoded unless under a `stringData` field. In the example before `RWxhc3RpYyBDbG91ZCBvbiBLOHMgKEVDSykK` base64 decodes to `Elastic Cloud on K8s (ECK)`.
