...
NOTE: When you change the `clusterIP` setting of the service, ECK will delete and re-create the service, as `clusterIP` is an immutable field. Depending on your client implementation, this might have a temporary impact on the ability to resolve the service's DNS name to endpoints.
