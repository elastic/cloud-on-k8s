...
NOTE: The operator applies default requests and limits for memory and CPU. They may be suitable for a first experience with the Elastic stack, however it is recommended to reevaluate these values for more advanced use cases.
...
The value set for cpu limits or requests directly impacts Elasticsearch `node.processors` setting. The following table gives the default value for `node.processors` given the cpu limits and requests set on the `elasticsearch` container:

.Default value for Elasticsearch `node.processors` setting depending on CPU `requests` and `limits`
