...
Staring with Elasticsearch 7.11 and later, the JVM heap size is automatically adjusted. For older Elasticsearch versions, you need to follow the instructions on <<{p}-jvm-heap-size,how to configure the JVM heap size>>.
