...
Until version 7.14.0 and ECK 2.10.0, Elastic Agent in Fleet mode has to run as root and in the same namespace as the Elasticsearch cluster it connects to. Since ECK 2.10.0, Agent in Fleet mode is required to be greater than version 7.14.0. Both the Fleet Server and the Elastic Agent are not required to run as root but there are some known issues when <<{p}_storing_local_state_in_host_path_volume>> that must be noted.

Due to current configuration limitations on Fleet/Elastic Agent side prior to version 7.14.0, ECK prior to version 2.10.0 needs to establish trust between Elastic Agents and Elasticsearch. ECK can fetch the required Elasticsearch CA correctly if both resources are in the same namespace.
...
tag::elastic-agent-fleet-non-root-daemonset-example[]
...
end::elastic-agent-fleet-non-root-daemonset-example[]
...
<1> This entry must not exist when running agent+fleet as non-root.
<2> That the correct URL for Elasticsearch is https://ELASTICSEARCH_NAME-es-http.YOUR-NAMESPACE.svc:9200
<3> The correct path for this is /mnt/elastic-internal/elasticsearch-association/YOUR-NAMESPACE/ELASTICSEARCH-NAME/certs/ca.crt