...
ECK uses environmental variables to control how Elastic Agent and Fleet Server should be configured. Sometimes, it might be required to override some of those settings. For example, if Kibana TLS certificate is signed by a well-known root and can't include `kibana-kb-http.namespace.svc` as a SAN, `KIBANA_FLEET_HOST` can be overriden to point to the URL that the certificate specifies. To do that, specify environmental variable, as below.
...
List of all environmental variables that can be used is available in Elastic Agent link:https://www.elastic.co/guide/en/fleet/current/agent-environment-variables.html[docs].
