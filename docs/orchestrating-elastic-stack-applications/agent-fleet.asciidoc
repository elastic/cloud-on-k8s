...
Until version 7.14.0 and ECK version 2.10.0, Elastic Agent and Fleet are required to run as root.
...
Elastic Agent in Fleet mode can only be deployed in the same namespace as the Fleet Server it is connecting to. This is due to the fact that the Elastic Agent does not have access to the Secret that contains the Elasticsearch CA when it is not running within the same namespace.
