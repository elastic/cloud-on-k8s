...
Similar to the <<{p}-deploy-elasticsearch,quickstart>>, the following section describe how to setup a simple Elasticsearch cluster specification, with one Elasticsearch node, managed by ECK, and how to customize an Elasticsearch deployment using the eck-stack Helm chart.
