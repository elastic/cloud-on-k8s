...
* <<{p}-logstash-http-config,HTTP Configuration>>
...
HTTPS is on by default in GA version. The link:https://www.elastic.co/guide/en/logstash/current/monitoring-logstash.html#monitoring-api-security[Logstash Monitoring APIs] are secured by having `api.ssl.enabled: true`, `api.ssl.keystore.path` and `api.ssl.keystore.password` in the `logstash.yml` by default.

You can also require HTTP Basic authentication by setting `api.auth.type: basic`, and providing the relevant credentials `api.auth.basic.username` and `api.auth.basic.password`:
...
If you want to use your own certificate, the required configuration is similar to Elasticsearch. Check <<{p}-custom-http-certificate>>.
