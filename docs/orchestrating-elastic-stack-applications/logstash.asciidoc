...
Plugins that require persistent storage include any plugin that store state locally. Typically this will include a configuration parameter that includes the name `path` or `directory` - not including paths to static content, such as certificates or keystores. For example the `sincedb_path` setting for the `file`, `dead_letter_queue` and `s3` inputs, the `last_run_metadata_path` for the `JDBC` input, `aggregate_maps_path` for the `aggregate` filter and `temporary_directory` for the `s3` output, which is used to aggregate content before uploading to s3.
