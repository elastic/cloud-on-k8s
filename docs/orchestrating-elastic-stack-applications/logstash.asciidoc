...
TIP: Always specify the port in the URL when {LS} is connecting to an external Elasticsearch Cluster.
...
* Providing additional resources for plugins (read-only and writable storage)
* <<{p}-logstash-working-with-plugins-scaling>>
* <<{p}-logstash-working-with-plugin-considerations>>
* <<{p}-logstash-working-with-custom-plugins>>
...
These may be ConfigMaps or Secrets files that have a 1 MB limit, or larger assets such as JDBC drivers, that need to be stored in a PersistentVolume.

[id="{p}-logstash-working-with-plugins-small-ro"]
===== ConfigMaps and Secrets (1 MB max)

Each instance of a `ConfigMap` or `Secret` has a maximum size of `1 MB`. 
...
==== Larger read-only assets (1 MB+)

Some plugins require or allow access to static read-only files that exceed the `1 MB` limit imposed by ConfigMap and Secret. 
...
Add these files using one or more of these options:

* **PersistentVolume.** Create a PersistentVolume that points to the static content, and create a PersistentVolumeClaim to request storage to be used by the {ls} resource.
* **initContainer.** Use an `initContainer` to add the files before the main container starts. This approach may be appropriate when the static content is hosted remotely.
* **Custom docker image.** 

Check out <<{p}-bundles-plugins>> for more details on which option might be most suitable for you.

[id="{p}-logstash-pv"]
===== Adding files using a PersistentVolume

[id="{p}-logstash-ic"]
===== Adding files using an initContainer

This example creates an `initContainer` to download a PostgreSQL JDBC driver JAR file, places it in a volume mount accessible to the main `container`, and then uses it in a JDBC input in the pipeline configuration.
...
===== Adding files using a custom Docker image
...
// Note to authors and reviewers: Clarify MB/Mib and GB/Gib

{ls} on ECK by default supplies a small (`1.5Gib`) default persistent volume to each pod.
...
NOTE: When plugins use writable storage, each plugin must store its data in its own dedicated location.
...
// Note to authors and reviewers: Is `logstash-filter-*` a real filter???

Examples of aggregating filters include {logstash-ref}/plugins-filters-aggregate.html[`logstash-filter-aggregate`], `logstash-filter-*`, and any {logstash-ref}/plugins-filters-ruby.html[`logstash-filter-ruby`] implementations that perform aggregations.

[id="{p}-logstash-inputs-data-pushed"]
==== Input plugins: data pushed to the input

{ls} installations with inputs that only push data to them should be able to scale freely and have load spread across them horizontally.
...
// Note to authors and reviewers: right now, there doesn't appear to be any overlap. Should we keep this tip?

TIP: Use these guidelines _in addition_ to the general guidelines provided in <<{p}-logstash-working-with-plugins-scaling>>.


{logstash-ref}/plugins-integrations-logstash.html[Logstash integration plugin]::
...
{logstash-ref}/plugins-inputs-elastic_agent.html[Elastic_agent input] and {logstash-ref}/plugins-inputs-beats.html[Beats input] plugins::
Set the `ttl` value on Elastic Agent or Beat to ensure that load is distributed appropriately.
...
// NOTE TO AUTHORS/REVIEWERS: `logstash-filter-elastic_integration` is a default plugin as of 8.13.0. Sub out and replace with another good example? 

This sample Dockerfile installs the {logstash-ref}/plugins-filters-tld.html[`logstash-filter-tld`] and {logstash-ref}/plugins-filters-elastic_integration.html[`logstash-filter-elastic_integration`] plugins to the official {ls} Docker image:
...
CAUTION: The examples in this section are for illustration purposes only and should not be considered to be production-ready. 
