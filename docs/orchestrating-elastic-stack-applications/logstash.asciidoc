...
Logstash keystore requires a password in an environment variable called `LOGSTASH_KEYSTORE_PASS`.
...
----
<1> If no password is specified, ECK gives a default value "changeit" to `LOGSTASH_KEYSTORE_PASS`
