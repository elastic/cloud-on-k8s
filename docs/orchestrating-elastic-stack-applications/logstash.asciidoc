...
[id="{p}-logstash-custom-files-plugins"]
== Adding plugins and files

experimental[]

Running {ls} may require additional files, such as JAR files needed to load JDBC drivers when using a JDBC or JMS plugin, or you may wish to use a plugin not included in the default Logstash docker image. To do this, there are two options available - using an `initContainer` to install plugins and add files before the main container start, or creating a custom docker image that includes the installed plugins and additional files. See <<{p}-bundles-plugins>> for a run down of which option might be most suitable for you.

=== Adding a plugin

To add plugins, the `bin/logstash-plugin` utility needs to be run to add the plugin to the logstash pod before it starts.
...
This example installs the same plugin, but uses a custom image, see XXX for more details

First, create a Dockerfile based on the logstash docker image, running `bin/logstash-plugin install` to install the appropriate plugin(s)
...
Then after building and deploying the custom image (see <<{p}-custom-images>> for more details), include it in the {ls} manifest:
...
Some Logstash plugins, such as the JDBC input, JDBC static filter or the JMS input require additional JAR files to be added in order to run.

==== Using an initContainer

This example creates an `initContainer` to download a `postgres` JDBC Driver JAR file, and place it in a volume mount accessible to the main `container`, and then use it in a JDBC input in the pipeline configuration.
  ...
  pipelines:
    - pipeline.id: main
      config.string: |
        input {
          jdbc {
             jdbc_driver_library => "/usr/share/logstash/jars/postgresql.jar" <1>
...
First, create a Dockerfile based on the logstash docker image, and download the JDBC driver, storing it alongside the other JAR files in the Logstash classpath:
+
[subs="attributes,+macros,callouts"]
----
FROM docker.elastic.co/logstash/logstash:{version}
...
----
<1> Placing the JAR file in the `/usr/share/logstash/logstash-core/lib/jars` folder places it on Logstash's classpath.
...
----
<1> Providing the correct version is always required as ECK reasons about APIs and capabilities available to it based on the version field.
<2> Note that when you place the JAR file on the Logstash classpath, you do not need to specify the `jdbc_driver_library` location in the plugin configuration.
