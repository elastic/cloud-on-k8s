...
* <<{p}-logstash-working-with-plugins,Working wuth plugins>>
...
Logstash on ECK allows use of all the plugins that you can use with regular Logstash, but may require some additional setup, or impose some restrictions on how Logstash pods can be configured.

[id="{p}-logstash-working-with-plugins-storage"]
=== Working with Plugins (Storage)

When working with plugins in Logstash configuration, there are certain considerations to take into account.

Plugins may require access to storage, and the nature of that access depends on the plugin, and the configuration option. Some plugins may require read-only volume mounts, which maybe serviceable by mounted ConfigMaps or Secrets with others requiring writable volumes

[id="{p}-logstash-working-with-plugins-read-storage"]
=== Working with Plugins (Read-only storage)

Often when configuring a logstash plugin to connect securely to a resource, the plugin configuration will include options for settings to configure the plugin with the necessary certificates, etc to communicate with the resource.

These are typically found in configuration settings such as `ssl_certificate`, `ssl_certificate_authorities`, `ssl_key`, `ssl_keystore_path`, `ssl_truststore_path`, where the setting calls for a `path`, or an `array` of `path` objects.

These settings expect to have a volume mounted with the appropriate content configured. Typically, these might be set using volume mounts configured with `ConfigMap`, `Secret` or a `Volume`
...
Other uses for read-only storage might be to include static content for use within Logstash. Various plugins include these static content for a variety of reasons, often for look up purposes. For example, this may include custom `grok` pattern definitions, source files for a `ruby` filter, a `dictionary` for a translate filter. Look for configuration settings that call for a `path` or an `array` of `paths`.

These settings expect to have a volume mounted with the appropriate content configured. Typically, these might be set using volume mounts configured with `ConfigMap`, `Secret` or a `Volume`. Note that `ConfigMap` and `Secret` have a maximum size of `1 MiB`. If your static content is larger than that,  you will likely need to mount a `Volume`, rather than a `ConfigMap` or `Secret`.
...
A plugin may also require Jar files, or other configuration files to be added to the Logstash classpath, or specified in the configuration using a path.

These again should be mounted as volumes, but these might be better configured using an `initContainer` or a new base image that downloads the content into that volume, or directly onto the classpath
...
[id="{p}-logstash-working-with-plugins-write-storage"]
=== Working with Plugins (Write storage)

Some plugins require access to write storage to function correctly, and store state related to the plugin work, either as a `sincedb` to keep track of the work already completed by the Logstash instance, and to avoid data duplication, or to store temporary files for outputs.

By default, Logstash supplies a small persisted `data` volume, which may be sufficient for storing temporary data, and can be mounted like
...
Additional data volumes can be be supplied in the `volumeClaimTemplates` section of the CRD, or the `data` volume
can be expanded (live volume expansion is supported where the underlying `StorageClass` supports Link to storage expansion)


[id="{p}-logstash-working-with-plugins-scaling-concerns"]
=== Working with Plugins (Scaling Concerns)

The ability to scale {ls} is highly dependent on the use of plugins used in pipelines. Not all {ls} deployments can be scaled horizontally by increasing the number of {ls} Pods defined in the Logstash resource.
...
When determining if Logstash can be scaled, take note of the input (and to a lesser extent, the filter) plugins.


Typically, pipelines where the input(s) are solely plugins that *receive* data from external sources should be able to be scaled when configured correctly. These are plugins, such as `logstash-input-beats`, `logstash-input-elastic_agent`, `logstash-input-http`, `logstash-input-tcp`, where Logstash acts as a "server". These can typically identified by a `host` configuration setting.
Other input plugins may introduce a restriction into how Logstash can be scaled, but still enable a Logstash resource to be scaled beyond a single pod. These are plugins where Logstash *retrieves* data from external sources, but does not store any state locally, as work is tracked in the resource itself. These types of inputs 
...
[id="{p}-logstash-working-with-input-plugins"]
=== Working with Input Plugins

Input Plugins have the most impact on a Logstash on ECK architecture
