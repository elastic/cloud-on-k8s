...
The {logstash-ref}/plugins-output-elasticsearch.html[`elasticsearch output`] plugin requires certain roles to be configured in order to enable {ls} to communicate with {es}.
...
==== Elastic_agent input and Beats input plugins

When you use the {logstash-ref}/plugins-inputs-elastic_agent.html[Elastic_agent input] or the {logstash-ref}/plugins-inputs-beats.html[Beats input],
set the {logstash-ref}/plugins-inputs-beats.html#_ttl[`ttl`] value on the input to ensure that load is distributed appropriately.
