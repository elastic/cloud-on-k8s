...
NOTE: Persistent Queues and Dead Letter Queues are not currently supported.
...
```
The permissions for this user <<{p}-users-and-roles,can be updated>> to include more indices if the Elasticsearch plugin is expected to use indices other than the default. See <<{p}-logstash-configuration-examples, Logstash configuration examples>> for a sample configuration that creates a user to write to a custom index.

The following example demonstrates how to create a Logstash deployment that connects to
