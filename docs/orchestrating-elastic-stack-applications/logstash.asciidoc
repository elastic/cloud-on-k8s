...
* <<{p}-logstash-securing-monitoring-endpoint,Securing Monitoring Endpoint>>
...
[id="{p}-logstash-securing-monitoring-endpoint"]
== Securing Monitoring Endpoint
...
NOTE: The variable substitution in `config` does not support the syntax of default value.
