...
For advanced troubleshooting it can be necessary to capture a JVM heap dump. The Enterprise Search Docker image is by default not configured to run with a data volume by the ECK operator. You can however write a heap dump to the `tmp` directory Enterprise Search uses. Please note that your heap dump will be lost if you do not manage to extract it before the container restarts.


[source,sh,subs="attributes,+macros"]
----
kubectl exec $POD_NAME \
 -- bash -c  'jmap -dump:format=b,file=tmp/heap.hprof $(jps| grep Main | cut -f 1 -d " ")'
