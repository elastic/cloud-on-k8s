...
Workplace Search requires an Enterprise license on ECK. You can start with a 30-day link:k8s-licensing.html[trial license].
...
+
NOTE: Your browser will show a warning because the self-signed certificate configured by default is not verified by a known certificate authority and not trusted by your browser. Acknowledge the warning for the purposes of this quickstart, but for a production deployment we recommend to <<{p}-enterprise-search-expose,configure valid certificates>>.
+
...
In the Enterprise Search YAML specification, you can change settings and version upgrades. ECK detects those changes and ensures a smooth rolling upgrade of all Enterprise Search Pods.
