...
Starting from ECK `2.6.0` and Elasticsearch `8.6.0`, Elastic Stack configuration policies let you configure in a declarative way, for multiple clusters, the following settings:
...
A policy can be applied to all or a subset of Elasticsearch clusters in a given namespace or all namespaces managed by the operator.

Settings described in a policy applied by the ECK operator are immutable through the Elasticsearch REST API.

It is not allowed to configure an Elasticsearch cluster with more than one policy currently.

[float]
[id="{p}-{page_id}-definition"]
== Define Elastic Stack configuration policies

Elastic Stack configuration policies can be defined in an `StackConfigPolicy` resource. Each `StackConfigPolicy` must have the following fields:

* `name` is a unique name used to identify the policy.
* `spec.elasticsearch` describes the settings to configure, at least one setting must be defined. Each following field is a named entity map:
  ** `clusterSettings` are the settings commonly found in elasticsearch.yaml settings.
  ** `snapshotRepositories` are snapshot repositories, to define off-cluster storage location for your snapshots.
...
* `namespace` is the namespace of the `StackConfigPolicy` resource, it is used to identify the Elasticsearch clusters to which this policy applies. If it equals to the operator namespace, all namespaces managed by the operator are used, otherwise only the policy namespace is used.
...
* `secureSettings` is a list of Secrets containing Secure Settings to inject into the keystore of the Elasticsearch to which this policy applies, similar to the<<{p}-custom-images,Elasticsearch Secure Settings>>.
...
[source,sh]
----
kubectl get -n b scp test-2-stack-config-policy -o jsonpath="{.status}" | jq .
----
