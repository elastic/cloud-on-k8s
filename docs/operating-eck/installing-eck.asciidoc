...
You can migrate an existing operator installation to Helm by doing the following:
...
. Uninstall the current ECK operator. You can do this by taking the `all-in-one.yaml` file you used to install the operator, deleting the CRD definitions from it and then running `kubectl delete -f all-in-one-without-crds.yaml`. Alternatively, you could delete each resource individually. For example, to uninstall version 1.2.1 of the operator, you can run the following command:
+
[source,sh]
----
