...
The `all-in-one.yaml` method is the quickest way to get started with ECK if you have full administative access to the Kubernetes cluster. The <<{p}-quickstart>> document describes how to proceed with this method. When you run the `kubectl` command listed in <<{p}-deploy-eck>>, the following components are installed or updated:
...
[source,sh]
----
helm install elastic-operator-crds eck-operator-crds 
----
