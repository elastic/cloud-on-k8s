...
Once the ECK operator image is copied internally, replace the original image name +docker.elastic.co/eck/eck-operator:{eck_version}+ with the private name of the image, for example +my-registry/eck/eck-operator:{eck_version}+, in the <<{p}-install-yaml-manifests,operator manifests>>. When using <<{p}-install-helm,Helm charts>>, you have to replace the `image.repository` Helm value with, for example, `my-registry/eck/eck-operator`.
