...
If the ECK Operator was installed using the YAML manifests, then a new `ValidatingWebhookConfiguration` needs to be installed. Apply the following manifest to enable the CRD deletion prevention webhook:
...
TODO: update the ECK configmap to enable the webhook, and restart the operator.

=== If using Helm

[source,sh]
----
helm install elastic-operator elastic/eck-operator -n elastic-system --create-namespace --set=webhook.crdDeletionProtection.enabled=true
----
