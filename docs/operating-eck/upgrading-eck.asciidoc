...
This release moves the link:https://kubernetes.io/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/[CustomResourceDefinitions] (CRD) used by ECK to the v1 version. If you are upgrading from a previous version of ECK the new version of the CRDs have to replace the existing CRDs. If removing the current ECK installation is not an option because you have production workloads that must not be deleted, then the following approach is recommended:

[source,shell,subs="attributes,callouts"]
.If you are using the the "all-in-one" manifest: replace existing CRDs
----
kubectl replace -f https://download.elastic.co/downloads/eck/{eck_version}/all-in-one.yaml
----

[source,shell,subs="attributes,callouts"]
.If you are using Helm: force upgrade the CRD chart to move to the v1 CRDs
----
helm upgrade eck-operator elastic/eck-operator-crds -n elastic-system --force
----

Then upgrade the main chart as usual:
[source,shell,subs="attributes,callouts"]
----
helm upgrade eck-operator elastic/eck-operator -n elastic-system
----

If you are using ECK through an OLM-managed distribution channel like operatorhub.io or the OpenShift OperatorHub then the CRD version upgrade will by handled by OLM for you and you do not need to take special action.
