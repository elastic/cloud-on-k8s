...
|set-default-security-context |true | Enables adding a default Pod Security Context to Elasticsearch Pods in Elasticsearch `8.0.0` and above. The default added sets `fsGroup` field to `1000` to match Elasticsearch container default UID. This behavior might not be appropriate for OCP and PSP-secured Kubernetes clusters, so it can be disabled.
