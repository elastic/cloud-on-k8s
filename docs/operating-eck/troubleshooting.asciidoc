...
When debugging, you might need to temporarily exclude an Elastic stack resource from management by ECK. When you do that no Kubernetes object depending on that stack resource is modified while you investigate.
To do this, set the annotation `eck.k8s.elastic.co/managed` to `false` on any resource controlled by the operator:
