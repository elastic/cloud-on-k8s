...
In exceptional cases it can be necessary to suspend the Elasticsearch process while using `kubectl exec` (<<{p}-exec-into-containers,see previous section>>) to troubleshoot.
One such example where Elasticsearch has to be stopped are the unsafe operations on Elasticsearch nodes that can be executed with the `https://www.elastic.co/guide/en/elasticsearch/reference/current/node-tool.html[elasticsearch-node]` tool.

To suspend an Elasticearch node, while keeping the corresponding Pod running, you can annotate the Elasticsearch resource with the `eck.k8s.elastic.co/suspend` annotation. The value should be a comma-separated list of the names of the Pods who's Elasticsearch process you want to suspend.
