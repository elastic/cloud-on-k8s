...
In exceptional cases, you might need to suspend the Elasticsearch process while using `kubectl exec`. Check the (<<{p}-exec-into-containers,previous section>>) to troubleshoot.
