...
Advanced users can force an upgrade by manually deleting `Pods`. The deleted `Pods` are automatically recreated at the latest revision. There are two exceptions to this rule:
