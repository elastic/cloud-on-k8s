...
|===
| Field | Description
| *`allocationDelay`* __string__ | AllocationDelay controls how long Elasticsearch will wait for the node to restart and join the cluster before reassigning its shards to other nodes. Only valid during restarts as part of a rolling upgrade of Elasticsearch 7.15.2 or newer.
|===
