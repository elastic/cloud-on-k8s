...
| *`type`* __ObjectType__ | Type determines the type of the Kubernetes object. Defaults to empty, which indicates the object is a local Elastic resource managed by ECK in the local Kubernetes cluster. Other option is 'Secret'. 'Secret' indicates the object is a Secret holding information (URL, username, password, CA cert) to reach an Elastic resource not managed by ECK.
|===
