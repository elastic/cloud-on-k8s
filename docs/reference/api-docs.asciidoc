...
[id="{anchor_prefix}-github-com-elastic-cloud-on-k8s-pkg-apis-common-v1-localselector"]
=== LocalSelector 

LocalSelector defines a reference to a Kubernetes object corresponding to an Elastic resource managed by the operator
...
| *`elasticsearchRef`* __xref:{anchor_prefix}-github-com-elastic-cloud-on-k8s-pkg-apis-common-v1-localselector[$$LocalSelector$$]__ | ElasticsearchRef is a reference to an Elasticsearch cluster running within the same k8s cluster.
|===
