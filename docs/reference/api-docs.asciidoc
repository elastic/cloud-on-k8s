...
[id="{anchor_prefix}-github-com-elastic-cloud-on-k8s-pkg-apis-elasticsearch-v1-restart"]
=== Restart 
...
|===
| Field | Description
| *`allocationDelay`* __string__ | AllocationDelay controls how long Elasticsearch will wait for the node to restart and join the cluster before reassigning its shards to other nodes. Only valid during restarts as part of a rolling upgrade of Elasticsearch 7.15.2 or newer. Values are expected to be Elasticsearch time units https://www.elastic.co/guide/en/elasticsearch/reference/8.1/api-conventions.html#time-units
|===
...
| *`restart`* __xref:{anchor_prefix}-github-com-elastic-cloud-on-k8s-pkg-apis-elasticsearch-v1-restart[$$Restart$$]__ | Restart defines settings applicable during Elasticsearch node restarts.
|===
