...
| *`preset`* __PresetName__ | Preset specifies which built-in configuration the operator should use. The configuration provided in a preset consists of: Beat config, roles with permissions required by that config and podTemplate for DaemonSet or Deployment. Preset must match the Beat `type`. If `config` is provided, it replaces the config from preset entirely. If `deployment` or `daemonSet` is provided it has to match Deployment or DaemonSet in the preset. The `podTemplate` is then merged with PodTemplate from the preset. If preset is not provided, both `config` and `daemonSet` or `deployment` must be specified.
...
[id="{anchor_prefix}-github-com-elastic-cloud-on-k8s-pkg-apis-beat-v1beta1-presetname"]
=== PresetName (string) 
