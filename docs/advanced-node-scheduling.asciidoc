...
Elasticsearch nodes can be configured with link:https://www.elastic.co/guide/en/elasticsearch/reference/current/modules-node.html[one or multiple roles]. This allows you describe an Elasticsearch cluster with, for example, 3 dedicated master nodes:
...
To avoid scheduling several Elasticsearch nodes from the same cluster on the same host, use a `podAntiAffinity` rule based on the the hostname and the cluster name label:
...
- node affinity for each group of nodes set to match the Kubernete nodes zone.
