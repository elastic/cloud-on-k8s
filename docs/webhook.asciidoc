...
It is the resource that defines the validating webhook. In ECK it is named `elastic-es-validation.k8s.elastic.co`.
It must be created before to start the operator, the `caBundle` field can be automatically managed as part of the
...
=== Webhook Server
It is the component that actually validates the submitted resources. In ECK it is the operator itself when it
is configured to act the `webhook` role. See <<{p}-operator-config,Configuring ECK>> if want to learn more about the
`operator-roles` flag.

=== The Certificates
A Secret contains the required certificates to secure the connection between the API server and the webhook.
