...
[float]
[id="{p}-2130-operator-statefulset-labels-annotations"]
=== Operator StatefulSet labels and annotations

ECK 2.13.0 operator helm chart supports adding custom annotations and labels to the operator StatefulSet.
...
A new option that allows a user to disable the elastic user from being created upon Elasticsearch creation was introduced in ECK 2.13.0. A use case for this would be when an organization/user would prefer to manage all users/roles via SSO (SAML/IDP/LDAP/etc)
