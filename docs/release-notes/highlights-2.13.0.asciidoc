...
A new option that allows a user to disable the elastic user from being created upon Elasticsearch creation was introduce in ECK 2.13.0. A use case for this would be when an organization/user would prefer to manage all users/roles via SSO (SAML/IDP/LDAP/etc)
