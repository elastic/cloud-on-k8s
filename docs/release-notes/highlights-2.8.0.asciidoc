...
For Elasticsearch version above `8.0.0` the containers `SecurityContext` also include the following capabilities settings:
...
Starting with Elasticsearch `8.8.0`, `runAsNonRoot` is also enforced to `true`:
