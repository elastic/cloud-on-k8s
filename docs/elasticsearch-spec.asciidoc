...
By default, Elasticsearch is using memory mapping (mmap) to efficiently access indexes. Usually, default values for virtual address space on Linux distributions is too low for Elasticsearch to work properly, which may result in out of memory exceptions. To increase virtual memory please configure nodes of your Kubernetes cluster according to:
https://www.elastic.co/guide/en/elasticsearch/reference/current/vm-max-map-count.html

Optionally, you can select a different type of storage. Here you can find information about possible options:
