...
spec:
  updateStrategy:
    changeBudget:
      ...
      maxUnavailable: 1
----
`maxSurge`: Refers to the number of Pods that can be scheduled out of the total number of Pods in the currently applied specification. This setting is useful when the new specification has similar number of Pods, but different node sets. For example, when migrating Pods to a new node set. This setting prevents the operator from creating all new Pods before removing the old ones, by temporarily avoiding two times more Pods than the ones defined in the specification.
...
As these constraints are enforced immediately after the specification is applied, it is possible that the actual state of the cluster might violate these constraints. When the cluster is outside these constraints, the operator can add or remove nodes only with the purpose of getting closer to these constraints. When the cluster is within these constraints, the operator can either add or remove nodes, as long as the cluster remains within the predefined constraints.
...
* Currently, the operator has a limited understanding of the cluster topology and during the update it might not pick the optimal order of Pods being added or removed. This can block the update process and you need to specify a higher `maxSurge` than theoretically needed.
