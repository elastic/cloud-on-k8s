...
`maxSurge` refers to the number of Pods that can be scheduled above the total number of Pods in currently applied spec.
...
Note that these constraints are enforced immediately after spec is applied, which often means that the actual state of the cluster is in violation of these constraints. This is expected and the operator won't stop from moving the cluster towards respecting them.
