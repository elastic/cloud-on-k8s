...
As these constraints are enforced immediately after the specification is applied, it is possible the actual state of the cluster might violate these constraints. As long as it remains within the predefined constraints defined by `maxSurge` and `maxUnavailable`, the operator allows the cluster to scale up or down.
