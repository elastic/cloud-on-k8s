...
* <<{p}-checkout-samples,Check out the samples>>
...
NOTE: The default resource request is 2GB memory and 100m CPU. Your pod will be `Pending` if your cluster does not have enough resources.
...
NOTE: For testing purposes only, you can specify the option `-k` to turn off certificate verification.
