...
Check the link:https://github.com/elastic/cloud-on-k8s/blob/master/operators/config/samples/elasticsearch/elasticsearch.yaml[Elasticsearch sample] for more details on how the Elasticsearch resource can be customized.

The full description of each `CustomResourceDefinition` can be found link:https://github.com/elastic/cloud-on-k8s/tree/master/operators/config/crds[in the project repository].
