...
BASE_IMG ?= $(GCR_REGION_NAME)/$(GCLOUD_PROJECT)/$(subst _,,$(USER))
MGR_IMG ?= $(BASE_IMG)-manager
SNAP_IMG ?= $(BASE_IMG)-snapshotter
...
docker-build: unit
	docker build . -f ./cmd/manager/Dockerfile -t $(MGR_IMG):$(IMG_TAG)
	docker build . -f ./cmd/snapshotter/Dockerfile -t $(SNAP_IMG):$(IMG_TAG)
...
override EXTRASEDFLAG = "-e"
	 ...
	 sed -i '' $(EXTRASEDFLAG) 's@image: .*@image: '"$(MGR_IMG):$(IMG_TAG)"'@' config/default/manager_image_patch.yaml
	 sed -i '' $(EXTRASEDFLAG) 's@value: .*@value: '"$(SNAP_IMG):$(IMG_TAG)"'@' config/default/manager_image_patch.yaml

# Push the docker image
docker-push:
	docker push $(MGR_IMG):$(IMG_TAG)
	docker push $(SNAP_IMG):$(IMG_TAG)
...
purge-gcr-images:
	...
	@ for i in $(gcloud container images list-tags $(MGR_IMG) | tail +3 | awk '{print $$2}'); \
		do gcloud container images untag $(MGR_IMG):$$i; \
	done
	@ for i in $(gcloud container images list-tags $(SNAP_IMG) | tail +3 | awk '{print $$2}'); \
		do gcloud container images untag $(SNAP_IMG):$$i; \
	done
