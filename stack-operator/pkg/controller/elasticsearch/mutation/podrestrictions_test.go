...
func TestNewPodRestrictions(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: PodRestrictions{
				MasterEligiblePods: podListToMap([]corev1.Pod{masterPod}),
				DataEligiblePods:   podListToMap([]corev1.Pod{dataPod}),
			},
		},
	}
...
func TestPodRestrictions_CanRemove(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			podRestrictions: PodRestrictions{
				MasterEligiblePods: podListToMap([]corev1.Pod{masterPod, namedPod("bar")}),
			},
			args: args{
				pod: masterPod,
			},
		...
		},
		{
			...
			podRestrictions: PodRestrictions{
				DataEligiblePods: podListToMap([]corev1.Pod{dataPod, namedPod("bar")}),
			},
			args: args{
				pod: dataPod,
			},
...
func TestPodRestrictions_Remove(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "can remove",
			podRestrictions: PodRestrictions{
				MasterEligiblePods: podListToMap([]corev1.Pod{namedPod("foo"), namedPod("bar")}),
				DataEligiblePods:   podListToMap([]corev1.Pod{namedPod("foo"), namedPod("bar")}),
			},
			args: args{
				pod: namedPod("foo"),
			},
			want: PodRestrictions{
				MasterEligiblePods: podListToMap([]corev1.Pod{namedPod("bar")}),
				DataEligiblePods:   podListToMap([]corev1.Pod{namedPod("bar")}),
			},
		},
		{
