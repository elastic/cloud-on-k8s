...
*IMPORTANT: The generated `clusterserviceversion.yaml` hardcodes a `readinessProbe` on port 8081. Setting `ENABLE_PROBES=false` via `Subscription.spec.config.env`, or shadowing the `eck.yaml` ConfigMap to disable probes, will cause the operator pod to be permanently not ready.*
