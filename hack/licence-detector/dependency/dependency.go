...
type Info struct {
	...
	LicenceTextFile string `json:"licenceTextFile"`
}
...
// LicenceTextFile will be read relative to the parent directory of the file.
func LoadOverrides(file string) (Overrides, error) {
	...
	for {
		...
		if dep.LicenceTextFile != "" {
			licFile, err := securejoin.SecureJoin(rootDir, dep.LicenceTextFile)
			if err != nil {
