	...
	return test.StepList{
		...
		},
		{
			...
			Test: test.Eventually(func() error {
				...
				err := k.Client.Get(k8s.ExtractNamespacedName(&b.Agent), &agent)
				if err != nil && !apierrors.IsNotFound(err) {
					return err
				}
				if err == nil {
		...
		WithSteps(test.StepList{
			...
			test.Step{
				...
				Test: func(t *testing.T) {
					...
					err := k.Client.Get(k8s.ExtractNamespacedName(&b.Agent), &createdAgent)
					require.NoError(t, err)
	...
	return test.StepList{
		{
			...
			Test: test.Eventually(func() error {
				var agent agentv1alpha1.Agent
				if err := k.Client.Get(k8s.ExtractNamespacedName(&b.Agent), &agent); err != nil {
					return err
				}
	...
	return test.StepList{
		{
			...
			Test: func(t *testing.T) {
				var agent agentv1alpha1.Agent
				require.NoError(t, k.Client.Get(k8s.ExtractNamespacedName(&b.Agent), &agent))
				agent.Spec = b.Agent.Spec
				require.NoError(t, k.Client.Update(&agent))
			},
		}}
}
