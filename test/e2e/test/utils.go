...
func deleteTestResources(ctx context.Context) error {
	...
	for _, namespace := range Ctx().Operator.ManagedNamespaces {
		...
		for _, secret := range list.Items {
			...
			deleted, err := deleteHelmReleaseFromSecret(secretName, namespace)
			if err != nil {
				return fmt.Errorf("while deleting secret %s of helm release in namespace %s: %w", secretName, namespace, err)
			}
...
func deleteHelmReleaseFromSecret(secretName string, namespace string) (bool, error) {
