...
func TestClientAuthRequired_FleetServerToAgent(t *testing.T) {
	...
	// Wrap the agent builder to verify auto-generated client cert is configured.
	agentWrapped := test.WrappedBuilder{
		BuildingThis: agentBuilder,
		PostCheckSteps: func(k *test.K8sClient) test.StepList {
			return test.StepList{
				// Verify 1 client certificate exists (from the elastic-agent connecting to fleet-server)
				clientauth.CheckClientCertificatesCountStep(k, namespace, fleetServerBuilder.Agent.Name, 1),
				{
					Name: "Verify fleet-managed Agent has auto-generated fleet-server client cert configured",
					Test: test.Eventually(func() error {
						var ag agentv1alpha1.Agent
						if err := k.Client.Get(context.Background(), types.NamespacedName{
							Namespace: namespace,
						...
						for _, assoc := range ag.GetAssociations() {
							...
							if conf == nil || !conf.ClientCertIsConfigured() {
								return fmt.Errorf("fleet-managed Agent should have an auto-generated fleet-server client cert configured")
							}
							return nil
						}
						return fmt.Errorf("fleet-managed Agent has no Fleet Server association")
