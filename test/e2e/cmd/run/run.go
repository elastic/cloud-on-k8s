...
func (h *helper) dumpK8sData() {
	...
	cmd := exec.Command("hack/diagnostics-utility/eck-dump.sh", "-N", operatorNs, "-n", managedNs, "-o", h.testContext.TestRun, "-z")
	err := cmd.Run()
	if err != nil {
		log.Error(err, "Failed to run hack/diagnostics-utility/eck-dump.sh")
	}
}
