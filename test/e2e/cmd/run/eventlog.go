...
func newEventLogger(client *kubernetes.Clientset, testCtx test.Context, logFilePath string) *eventLogger {
	eventWatch := cache.NewListWatchFromClient(client.EventsV1().RESTClient(), "events", metav1.NamespaceAll, fields.Everything())
	el := &eventLogger{
...
func (el *eventLogger) runEventProcessor() {
	...
	for {
		...
		evt := evtObj.(*eventsv1.Event) //nolint:forcetypeassert
...
func (el *eventLogger) isInterestingEvent(evt *eventsv1.Event) bool {
	...
	if _, exists := el.interestingNamespaces[evt.Regarding.Namespace]; exists {
